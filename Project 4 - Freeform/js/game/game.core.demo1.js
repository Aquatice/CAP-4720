	...
	var _game = {
		...
		player: {
			...
			updateOrientation: function() {
				...
				_game.player.rotationAngleY = Math.round(window.game.helpers.radToDeg(_game.player.rotationRadians.y))+180;

				// Prevent player from being upside-down on a slope - this needs improvement
				if ((_cannon.getCollisions(_game.player.rigidBody.index) &&
					((_game.player.rotationAngleX >= 360) ||
						(_game.player.rotationAngleX <= -360) ||
						(_game.player.rotationAngleY >= 360) ||
						(_game.player.rotationAngleY <= -360)))
					)
				{
					// Reset orientation
